...
const GemstoneBracelets = () => {
  ...
  useEffect(() => {
    console.log('GemstoneBracelets: Setting up real-time listener...');
    setLoading(true);
    
    const unsubscribe = subscribeToProductsBySubcategory('Bracelets', (fbProducts) => {
      console.log('GemstoneBracelets: Real-time update received:', fbProducts.length, 'products');
      const uiProducts = adaptFirebaseArrayToUI(fbProducts);
      const gemstoneBracelets = uiProducts.filter(product => 
      ...
      setProducts(gemstoneBracelets);
      
      if (gemstoneBracelets.length > 0) {
        const prices = gemstoneBracelets.map(p => p.price);
          ...
          <div className="relative z-20 text-center px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex items-center justify-center gap-2 text-sm md:text-base text-white/90">
                ...
                <span>{'>'}</span>
                <span className="text-white font-medium">GEMSTONE BRACELETS</span>
              </div>
            </motion.div>
          </div>
        </section>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <aside className="lg:w-64 flex-shrink-0">
                    ...
                    {Object.entries(categoryCounts).map(([category, count]) => (
                      <label key={category} className="flex items-center justify-between cursor-pointer group">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-sm text-foreground group-hover:text-primary transition-colors">{category}</span>
                        </div>
                ...
                <div className="bg-background border border-border rounded-lg p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <button onClick={() => setPriceRange([minPrice, maxPrice])} className="w-full py-2 px-4 bg-foreground text-background text-sm font-medium rounded-lg hover:bg-foreground/90 transition-colors">Reset Filter</button>
                  </div>
                </div>
              </aside>
              ...
              {loading ? (
                ...
                <div className="col-span-full py-16 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">No Gemstone Bracelets Available</h3>
...
export default GemstoneBracelets;
